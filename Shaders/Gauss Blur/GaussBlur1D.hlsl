...
PS_OUTPUT ps_main( in PS_INPUT In)
{
	...
	if(!Dir){
		for(int x=-R;x<=R;x++){
			float offset=R*fPixelWidth;
			float2 pos=In.texCoord-float2(-offset+R*fPixelWidth,0)+float2(x*fPixelWidth,0);	
			Sum.rgb+=(bkd.Sample(bkdSampler, pos)).rgb*weight[x+R];		
			// if((pos.x==PositionEdge(pos).x)&&(pos.y==PositionEdge(pos).y)){
			// 	Sum.rgb+=(bkd.Sample(bkdSampler, PositionEdge(pos))).rgb*weight[x+R];
			// }
			// else{

			// }
			//Sum.rgb+=(bkd.Sample(bkdSampler, PositionEdge(In.texCoord-float2(R*fPixelWidth,0)+float2(x*fPixelWidth,0)))).rgb*weight[x+R];
		}
	}else{
		for(int y=-R;y<=R;y++){
			float offset=R*fPixelHeight;
			float2 pos=In.texCoord-float2(0,-offset+R*fPixelHeight)+float2(0,y*fPixelHeight);
			Sum.rgb+=(bkd.Sample(bkdSampler, pos)).rgb*weight[y+R];
			// if((pos.x==PositionEdge(pos).x)&&(pos.y==PositionEdge(pos).y)){
			// 	Sum.rgb+=(bkd.Sample(bkdSampler, PositionEdge(pos))).rgb*weight[y+R];
			// }else{

			// }
			//Sum.rgb+=(bkd.Sample(bkdSampler, PositionEdge(In.texCoord-float2(0,R*fPixelHeight)+float2(0,y*fPixelHeight)))).rgb*weight[y+R];
		}
	}

	//Out.Color.rgb = Sum.rgb;
	Out.Color.rgb = Sum.rgb*Out.Color.a+(bkd.Sample(bkdSampler, In.texCoord)).rgb*(1-Out.Color.a);
	Out.Color.a=1.0;
